...
if (require("modules")) {
  print("modules is loaded correctly")
} else {
  print("trying to install modules")
  install.packages("modules")
  if (require("modules")) {
    print("modules installed and loaded")
  } else {
...
CytoConverter <-
  function(in_data,
    ...
    {
      out <-
        try(parser(j, xmod, ymod, transloctable, addtot, Cyto_sample))
                ...
                Con_data[i, 2]))
      {
        ...
        for (j in startcol:length(Cyto_sample))
        {
          ...
          } else
          {
            ...
            inc_table <-
              colparse(j, xmod, ymod, transloctable, addtot, Cyto_sample)
...
input_m <-
  as.matrix(as.data.frame(
    read.delim(file = "input/CytoConverter_Example_File.txt", header = F, sep = "\t")
  ))
result <- CytoConverter(input_m, constitutional = F, guess = T)
write.table(
  result[[1]],
  file = 'output/CYTO_CONVERTED.TXT',
  quote = FALSE,
  sep = '\t',
  col.names = F,
  row.names = F
...
write.table(
  result[[2]],
  file = 'output/CYTO_CONVERTED_LOG.TXT',
  quote = FALSE,
  sep = '\t',
  col.names = F,
  row.names = F
