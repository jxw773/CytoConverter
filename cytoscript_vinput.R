...
CytoConverter<-function(in_data)
{
  ...
  getCytoBands <- function(lengthcount, o, temp,coln,derMods) {
    ...
    if (any(grepl(";;", temp[[lengthcount * 2]][o]) |
      ...
      for (j in 1:length(longform_table))
      {
        ...
        if (grepl("pter", positions[1]))
        {
          positions[1] <- chr_table_2[1, 4]
        }
        if (grepl("qter", positions[2]))
        {
          positions[2] <- chr_table_2[nrow(chr_table_2), 4]
        }
        ...
        ##change this to get compliment of this stuff
        positions_table <-
          grep(paste(positions, collapse = ":", sep = "|"), chr_table_2[, 4])
      ...
      for (lengthcount in 1:(if(!is.integer((length(temp) / 2))){ceiling(length(temp)/2)}else{length(temp)/2}))
      {
        ...
        if (grepl("ins\\(", derMods[(lengthcount * 2-1)])) {
          ...
          if (!any(grepl(inschrom, names(transloctable))))
          {
            ...
            currentvec <- c(currentvec,getCytoBands(lengthcount, 1,temp))
            ...
            currentvec <- c(currentvec,getCytoBands(lengthcount, 2,temp))
  ...
  mergeTable<-function(M){
    ...
    if(length(wg)>1){
      ...
      for(j in 1:(nrow(Mg)-1)){
        Mg[j:nrow(Mg),]<-mergeBelowL(Mg[j:nrow(Mg),])}
      M[wg,1:3]<-Mg}
    
    ##if(length(wl)>1){
    ##Ml<-M[wl,1:3]
    ##for(j in 1:(nrow(Ml)-1)){
    ##Ml[j:nrow(Ml),]<-mergeBelow(Ml[j:nrow(Ml),])}
    ##M[wl,1:3]<-Ml}
  ...
  rowparse<-function(i){
    ...
    ##sorted_sample_table<-as.data.frame(sorted_sample_table,row.names=FALSE)
    ##sorted_sample_table[,1]<-as.character(sorted_sample_table[,1])
    ##sorted_sample_table[,2]<-as.numeric(as.character(sorted_sample_table[,2]))
    ##sorted_sample_table[,3]<-as.numeric(as.character(sorted_sample_table[,3]))
    ##sorted_sample_table[,4]<-as.character(sorted_sample_table[,4])
  ...
  for (i in 1:length(IDs)) {
    ...
    w <- grep(paste("^", IDs[i], "_", sep = ""), out[, 1])
    ##if (length(w) > 1) {
...
return(list(Final_Final_table,Dump_table))
}