                   ...
                   forMtn)
{
  ...
  if (length(temp) == 1) {
    ...
    {
      transchrom <-
        str_extract(Cyto_sample[coln], paste(gsub(
          "\\?", "\\\\?", gsub("\\+", "\\\\+", gsub("\\(", "\\\\(", derMods[1]))
        ), "\\)\\(.+?\\)", sep = ''))
    ...
    repeat {
          ...
          })
      {
        ...
        if (any(grepl("^r\\(", derMods[lengthcount * 2 - 1])) &
          ...
          derModsRing <-
            ...
            paste("(", paste(tempRingPosition, collapse = ";"), sep = ""))
          tempstorage <- NULL
          tempdermods <- NULL
          ...
          if ((length(temp)) > (lengthcount * 2))
          {
            ...
            tempdermods <-
              derMods[((lengthcount * 2)):length(temp)]
        ...
        if (grepl("(t|ins)\\(", derMods[(lengthcount * 2 - 1)]))
        {
          ...
          transchrom <-
            str_extract(Cyto_sample[coln], gsub(
              "\\?",
              "\\\\?",
              paste(gsub(
          ...
          {
            transchrom <-
              str_extract(Cyto_sample[coln], gsub("\\?", "\\\\?", paste(
                gsub("\\+", "\\\\+", gsub("\\(", "\\\\(", derMods[(lengthcount * 2 - 1)])), "\\)", sep =
                  ''
              )))
          }
              ...
              !grepl("der", Cyto_sample[coln]))
          {
            ...
            selectedTransTable <-
              as.matrix(transloctable[grep((gsub(
                "\\)", "\\\\)", gsub("\\(", "\\\\(", transchrom)
              )), names(transloctable))][[1]])
            transDer <-
              selectedTransTable[grep(paste(
                "der\\(",
                paste(
            ...
            temp <-
              c(if ((lengthcount * 2 - 1) != 1) {
                ...
                transtemp[1]
              }, transtemp[2], if ((length(temp) >= lengthcount * 2) &&
      ...
      if (grepl("ins\\(", derMods[(lengthcount * 2 - 1)])) {
        ...
        if (length(temp[[lengthcount * 2 - 1]]) == 1 &&
            any(length(temp[[(lengthcount * 2 - 1)]]) < str_count(temp[[lengthcount *
                                                                        2]], "p|q")))
        {
          temp[[(lengthcount * 2 - 1)]] <-
          ...
          temp[[lengthcount * 2]] <-
            c(str_sub(temp[[lengthcount * 2]], 1, splitloc - 1),
              str_sub(temp[[lengthcount * 2]], splitloc, str_length(temp[[lengthcount *
                                                                            2]])))
          within = T
        }
...
mergeTable_jp <- function(M, keep_extras = F) {
  ...
  insertSection <- function(h_, start, end, type) {
    ...
    for (start_val in start_vals) {
      ...
      if (start_val >= start &
        ...
        h_[[as.character(start_val)]][[type]][[length(h_[[as.character(start_val)]][[type]]) +
                                                 1]] <- end
      ...
      if (gain == ploss & gain == loss) {
        ...
        if (min_val > 0) {
          for (i in 1:min_val) {
            ...
            h_[[as.character(start_val)]][['+Loss']][[1]] <-
              NULL
          }
        }
    ...
    getContiguousSection <- function(h__, section, orig_end) {
      ...
      if (has.key(as.character(sect_end), h__)) {
        ...
        if (orig_end %in% h__[[sect_end]][[section[['Type']]]]) {
          ...
          orig_end_index <-
            match(orig_end, h__[[sect_end]][[section[['Type']]]])
          h__[[sect_end]][[section[['Type']]]][[orig_end_index]] <-
            NULL
          ...
          section <-
            getContiguousSection(h__, section, orig_end)
      ...
      while (length(start_vals)) {
        ...
        if (length(start_index) > 0) {
          ...
          if (length(end_index) > 0) {
            ...
            coord_table[end_index[1], ][['Start']] <-
              section[['Start']]
          } else {
            # add new section to table
            coord_table <-
              rbind(coord_table, as.data.frame(t(section)))
          }
        }
  ...
  for (row in 1:nrow(M)) {
    if (M[row, 'Type'] == 'Gain' |
        M[row, 'Type'] == 'Loss' | M[row, 'Type'] == '+Loss') {
      h[M[row, 'Chr']] <- insertSection(h[[M[row, 'Chr']]],
...
mergeDeletions <- function(M, Mainchr) {
  OldM <- M
  startDel <-
    grep("((t\\()|(idic\\()|(rob\\()|(trc\\()|(dic\\()|(Gain))",
         M[, 4])[1]
  
  
  wdel <-
    union(intersect(grep("del", M[, 4]), grep("^del", M[, 4], invert = T)), intersect(grep("add", M[, 4]), grep("^add", M[, 4], invert =
                                                                                                                  T)))
  ...
  if (!is.na(startDel))
  {
    ...
    wdel <-
      setdiff(union(intersect(
        grep("del", M[, 4]), grep("^del", M[, 4], invert = T)
      ), intersect(
        grep("add", M[, 4]), grep("^add", M[, 4], invert = T)
      )), startDel)
...
rowparse <- function(Cyto_ref_table, ref_table, Cyto_sample, Con_data, i, guess, guess_q, orOption, constitutional, transloctable, Dump_table, sexstimate, forMtn) {
  sample_table <- matrix(ncol = 4, nrow = 0)
  colnames(sample_table) <- c("Chr", "Start", "End", "Type")
  sorted_sample_table <- matrix(ncol = 4, nrow = 0)
  colnames(sorted_sample_table) <-
            ...
            Con_data[i, 2]))
  {
              ...
              Con_data[i, 2]))
    {
      ...
      else if ((sexstimate == F &&
                any(grepl("\\?", Cyto_sample[2])))) {
        normX = 2
        normY = 0
      } else
      {
    ...
    for (j in startcol:length(Cyto_sample))
    {
      ...
      } else
      {
        ...
        inc_table <-
          colparse(Cyto_ref_table, ref_table, j, xmod, ymod, transloctable, addtot, Cyto_sample, guess_q, constitutional, forMtn)
        if (is.character(inc_table))
        {
          ...
          if (nrow(temp_table) > 0)
          {
            ...
            additionTable <-
              detectAdd(temp_table[, 4], if (nrow(ex_table) > 0) {
                ex_table[, 4]
              } else{
                NULL
              })
                ...
                any(!grepl("^t\\(|^ins\\(|^inv\\(", temp_table[, 4])))
            {
              ...
              if (grepl("^ids$", Cyto_sample[length(Cyto_sample)]))
              {
                ...
                {
                  tempindexchromtracker <-
                    sapply(gsub("\\$", "", Mainchr), function(x) {
                      if (!grepl("X|Y", x))
                  ...
                  if (!is.null(tempindexchromtracker) &&
            ...
            {
              mod = TRUE
              temp_table[intersect(grep("LongDer", temp_table[, 4]),
                                   grep(
            ...
            {
              if (nrow(temp_table) > length(union(((
                intersect(
              ...
              ))))
              {
                ...
                if (is.vector(temp_table))
                {
                  ...
                  temp_table[, 2:3] <-
                    as.numeric(temp_table[, 2:3])
                  colnames(temp_table) <-
                    c("Chr", "Start", "End", "Type")
                } else if (nrow(temp_table) == 4 &
                           ncol(temp_table) == 1)
                {
                  ##if its a translocated vector, fix now
                  temp_table <- t(temp_table)
                  temp_table <-
                    as.data.frame(as.list(temp_table))
                  temp_table[, 2:3] <-
                    as.numeric(temp_table[, 2:3])
                  colnames(temp_table) <-
                    c("Chr", "Start", "End", "Type")
            ...
            (length(temp_table[, 4]) > length(c(
              which(grepl("del", temp_table[, 4])), which(grepl("add", temp_table[, 4]))
            ))) &
            ...
            if (any(grepl("i\\(.*", temp_table[, 4])))
            {
              ...
              if (nrow(temp_table) > 0)
              {
                ...
                temp_table[grep("i\\(.*", temp_table[, 4]), 4] <-
                  "Gain"
                additionTable[[1]] <-
                  c(additionTable[[1]], additionTable[[1]][grep("i\\(.*", original_temp_table[, 4])])
                temp_table <-
                  rbind(temp_table, original_temp_table[grep("i\\(.*", original_temp_table[, 4]),])
            ...
            if (any(grepl("ider\\(.*", temp_table[, 4])))
            {
              ...
              if (nrow(temp_table) > 0)
              {
                ...
                temp_table[intersect(
                  ...
                  "Gain"
                additionTable[[1]] <-
                  c(additionTable[[1]], additionTable[[1]][intersect(
                    grep("ider\\(.*", temp_table[, 4]),
                    grep("del\\(.*|add\\(.*", temp_table[, 4], invert = T)
                  )])
                temp_table <-
                  rbind(temp_table, original_temp_table[grep("ider\\(.*", original_temp_table[, 4]),])
            ...
            if (any(grepl("idic\\(.*", temp_table[, 4])))
            {
              ...
              if (nrow(temp_table) > 0)
              {
                ...
                additionTable[[1]] <-
                  c(additionTable[[1]], additionTable[[1]][intersect(
                    grep(
                      "del\\(.*|add\\(.*",
                      original_temp_table[, 4],
                      invert = T
                    ),
                    grep("idic\\(.*", original_temp_table[, 4])
                  )])
                temp_table <-
                  rbind(temp_table, original_temp_table[intersect(
                    grep(
                      "del\\(.*|add\\(.*",
                      original_temp_table[, 4],
                      invert = T
              ...
              if (nrow(ex_table) > 0)
              {
                ...
                ex_table[grep("idic\\(.*", ex_table[, 4]), 4] <-
                  "Gain"
                additionTable[[2]] <-
                  c(additionTable[[2]], additionTable[[2]][grep("idic\\(.*", original_ex_table[, 4])])
                ex_table <-
                  rbind(ex_table, original_ex_table[grep("idic\\(.*", original_ex_table[, 4]),])
            ...
            if (any(grepl("trc\\(.*", temp_table[, 4])))
            {
              ...
              if (any(grepl("long", temp_table[, 4])))
              {
                if (nrow(ex_table) > 0)
                {
                  ex_table[intersect(grep("trc\\(.*", ex_table[, 4]),
                                     grep(
              ...
              } else
              {
                if (nrow(temp_table) > 0)
                {
                  temp_table[intersect(grep("trc\\(.*", temp_table[, 4]),
                                       grep(
                                         paste("chr", Mainchr[1], "chr", Mainchr[3], sep = '|'),
                                         temp_table[, 1]
                                       )), 4] <- "Loss"
              ...
              {
                ex_table[intersect(grep("trc\\(.*", ex_table[, 4]),
                                   grep("chr", Mainchr[2], ex_table[, 1])), 4] <-
                  "Loss"
                ex_table[, 4] <-
                  paste(additionTable[[2]], ex_table[, 4], sep = "")
              }
            ...
            if (any(grepl("trp\\(.*", temp_table[, 4])))
            {
              ...
              temp_table <-
                rbind(temp_table, temp_table[grep("trp\\(.*", temp_table[, 4]),])
            ...
            if (any(grepl("qdp\\(.*", temp_table[, 4])))
            {
              ...
              temp_table <-
                rbind(temp_table, temp_table[grep("qdp\\(.*", temp_table[, 4]),], temp_table[grep("qdp\\(.*", temp_table[, 4]),])
            }
            ...
            if (nrow(temp_table) > 0)
            {
              ...
              temp_table[grep("\\+Gain", temp_table[, 4]), 4] <-
                "Gain"
              temp_table[intersect(grep("\\+", temp_table[, 4]),
                                   grep("\\+Loss", temp_table[, 4], invert = T)), 4] <-
                "Gain"
              ...
              Plus_Loss <-
                temp_table[grep("\\+Loss", temp_table[, 4]), ]
              
              ##cut off intersections early
              ###Think aboiut this
              if (length(Plus_Loss) > 0 &&
                  nrow(Plus_Loss) > 0 &&
                  any(grepl("Gain", temp_table[, 4]) &
                      any(grepl("Loss", temp_table[, 4]))))
              {
    ...
    } else
    {
      ...
      if (!is.null(val_remainder))
      {
        if (any(val_remainder == 0))
        {
          ...
          val_remainder <-
            val_remainder[grep(TRUE, val_remainder == 0)]
          
        } else if (any(val_remainder_2 == 0)) {
          val_divider <- val_divider_2[grep(TRUE, val_remainder_2 == 0)]
          val_remainder <-
            val_remainder_2[grep(TRUE, val_remainder_2 == 0)]
        } else{
                   ...
                   ))
        {
          ...
          diffval <-
            diffval[intersect(grep(TRUE, diffval / length(clone_chrom_tracker) > 0),
    ...
    if (constitutionalcount > 0 && constitutional == F) {
      ...
      if ((idealTotal * (ploidy - 2) / 2 + idealTotal) != (count_before_extras) |
          (
        ...
        } else if (count_before_extras + xdel + ydel > idealTotal)
        {
          ...
          if ((count_before_extras) + xdel + ydel > idealTotal &
              (xdel + ydel) > 0) {
...
CytoConverter <-
  function(in_data,
    ...
    {
      Cyto_ref_table <-
        sapply(as.data.frame(read.delim(
          "Builds/cytoBand_GRCh38.txt", header = FALSE
        )), as.character)
      
    } else if (build == "hg19") {
      Cyto_ref_table <-
        sapply(as.data.frame(read.delim("Builds/cytoBand_hg19.txt", header = FALSE)), as.character)
      
    } else if (build == "hg18") {
      Cyto_ref_table <-
        sapply(as.data.frame(read.delim("Builds/cytoBand_hg18.txt", header = FALSE)), as.character)
      
    } else if (build == "hg17") {
      Cyto_ref_table <-
        sapply(as.data.frame(read.delim("Builds/cytoBand_hg17.txt", header = FALSE)), as.character)
    ...
    ref_table <-
      as.data.frame(Cyto_ref_table[sapply(unique(Cyto_ref_table[, 1]), function(x) {
        grep(x, Cyto_ref_table[, 1])[length(grep(paste(x, "$", sep = ""), Cyto_ref_table[, 1]))]
      }), ][, c(1, 3)])
    ...
    colnames(Final_table) <-
      c("Sample ID", "Chr", "Start", "End", "Type")
    ...
    {
      Dump_table <-
        rbind(Dump_table, c(fish_table, "Warning in fish reading"))
      ##now take out ish readings
      in_data[, 2] <- gsub("ish.*$", "", as.character(in_data[, 2]))
      
    } else {
      Dump_table <-
        rbind(Dump_table, cbind(fish_table, "Warning in fish reading"))
      ##now take out ish readings
    ...
    for (i in 1:nrow(in_data))
    {
      ...
      if (!is.na(in_data[i, 2]) &&
        ...
        data_split <-
          unlist(strsplit(gsub("/", "/-;-;opj", data_split), "/"))
        mut_index <- grep("\\(.*\\)(?!\\()", data_split, perl = T)
        if (length(mut_index) > 0)
        {
          mut_list <-
            sapply(data_split[mut_index], function(x) {
              gsub("\\[|\\]", "", gsub("\\)", "\\\\)", gsub(
                "\\(", "\\\\(", gsub("\\?", "\\\\?", gsub("\\+", "", substr(
                  x, 2, nchar(x)
                )))
          ...
          index_match <-
            sapply(mut_list, function(x) {
              grep(x, data_split)[1]
            })
          index_match <-
            index_match[intersect(which(!is.na(index_match)), which(index_match > 0))]
          if (is.numeric(mut_index) &&
              is.numeric(index_match) && length(index_match) > 0)
          {
            additional_to_add <-
              substring(names(index_match), 0, regexpr("\\+", names(index_match)))
            data_split[mut_index] <-
              paste(additional_to_add , data_split[index_match], sep = "")
          }
        }
        
        in_data[i, 2] <-
          gsub(",-;-;opj", "/", paste(data_split, collapse = ","))
        ...
        if (!is.vector(temp_data) && nrow(temp_data) > 1)
        {
          for (j in 2:nrow(temp_data))
          {
            ...
            if (grepl("idem|sl", temp_data[j, 2]))
            {
              ...
              prev <-
                unlist(strsplit(temp_data[j - 1, 2], "\\["))[1]
            ...
            if (!is.null(autochromprev))
            {
              ...
              {
                cur <-
                  c(cur[1:2], rep(autochromprev, clonecount), cur[3:length(cur)])
              } else
              {
    ...
    unsure_table <-
      Con_data[grep("\\?|\\~|inc|mar|add", Con_data[, 2]),]
    ...
    for (i in 1:nrow(Con_data))
    {
      ...
      if (any(grepl("ids|idem|sl|sd", Cyto_sample)) ||
          allow_Shorthand)
      {
        ...
        {
          mut_list <-
            sapply(Cyto_sample[mut_index], function(x) {
              gsub("\\)", "\\\\)", gsub("\\(", "\\\\(", gsub(
                "\\?", "\\\\?", gsub("\\+", "", substr(x, 2, nchar(x)))
              )))
            })
          index_match <-
            sapply(mut_list, function(x) {
              ...
              is.numeric(index_match) && length(index_match) > 0)
          {
            ...
            Cyto_sample[mut_index] <-
              paste(additional_to_add , Cyto_sample[index_match], sep = "")
          }
        }
        ...
        {
          mut_list <-
            sapply(Cyto_sample[mut_gone_index], function(x) {
              gsub("\\)", "\\\\)", gsub("\\(", "\\\\(", gsub(
                "\\?", "\\\\?", substr(x, 2, nchar(x))
              )))
            })
          ...
          index_cancel <-
            index_cancel[intersect(which(!is.na(index_cancel)), which(index_cancel >
                                                                        0))]
          if (is.numeric(mut_gone_index) &&
              is.numeric(index_cancel) && length(index_cancel) > 0)
          {
      ...
      {
        tottable <- samparse(Cyto_ref_table, ref_table, Cyto_sample, Con_data, i, guess, guess_q, orOption, constitutional, transloctable, Dump_table, sexstimate, forMtn)
        ...
        } else if (!is.list(tottable))
        {
          ...
          if (nrow(Final_table) == 1)
          {
            ...
            Final_table[, 3] <-
              as.integer(as.numeric(as.character(Final_table[, 3])))
            Final_table[, 4] <-
              as.integer(as.numeric(as.character(Final_table[, 4])))
          ...
          if (i < nrow(Con_data))
          {
            ...
            if (curname[-length(curname)] == nextname[-length(nextname)] &
                as.numeric(curname[length(curname)]) + 1 == as.numeric(nextname[length(nextname)]))
            {
    ...
    ##remove duplicates and redundancy (same region loss/gain within group do this first, then duplicates)
    ...
    for (i in 1:length(IDs)) {
      ...
      w <-
        grep(gsub("\\+", "\\\\+", gsub(
          "\\*", "\\\\*", gsub("\\!", "\\\\!", paste("^", IDs[i], "_", sep = ""))
        )), out[, 1])
      ##if (length(w) > 1) {
      nxt <-
        unlist(lapply(strsplit(as.character(out[w, 2]), split = "\\["),
                      function(x) {
                        x[2]
                      }))
    ...
    colnames(Dump_table) <-
      c("Sample ID", "Karyotype", "Error Message")
