            ...
            Con_data[i, 2]))
  {
    ...
    for (j in startcol:length(Cyto_sample))
    {
      ...
      } else
      {
        ...
        inc_table <-
          mod_parser$colparse(Cyto_ref_table, ref_table, j, xmod, ymod, transloctable, addtot, Cyto_sample, guess_q, constitutional, forMtn)
        if (is.character(inc_table))
        {
          ...
          if (nrow(temp_table) > 0)
          {
            ...
            additionTable <-
              mod_utils$detectAdd(temp_table[, 4], if (nrow(ex_table) > 0) {
                ex_table[, 4]
              } else{
                NULL
              })
