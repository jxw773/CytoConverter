...
#' Prepare Data for Cytogenetic Visualization
#' 
#' @description
#' Processes CytoConverter results into a format suitable for plotting chromosomal
#' gains and losses. This function handles coordinate transformation, identifies
#' overlapping aberrations, and prepares the data structure needed for visualization.
#' 
#' @param cyto_list Data frame. Results from CytoConverter containing chromosomal
#'   aberrations with columns: Sample ID, Chr, Start, End, Type, Percent Present.
#' @param ref_list Character or matrix. Reference genome build ("GRCh38", "hg19", 
#'   "hg18", "hg17") or custom cytoband matrix. Default "GRCh38".
#' @param include_normals_graph Logical. Whether to include normal samples in the
#'   visualization data. Default FALSE.
#' @param list_of_samples Character vector. Sample identifiers to include when
#'   include_normals_graph=TRUE. Default NULL.
#'
#' @return List containing visualization data elements:
#'   \item{rect_maker}{Matrix of rectangle coordinates for plotting aberrations}
#'   \item{xbegin}{X-coordinate starting positions}
#'   \item{xcoord_master}{Master X-coordinate mapping}
#'   \item{y_above}{Y-coordinates for upper bounds}
#'   \item{y_below}{Y-coordinates for lower bounds}
#'   \item{sorted_reflist}{Sorted reference chromosome list}
#'   \item{cum_length_coords}{Cumulative chromosome length coordinates}
#'   \item{start_cum_length}{Starting cumulative lengths}
#'   \item{uniq_coord_name}{Unique sample identifiers}
#'
#' @examples
#' # Basic usage with CytoConverter output
#' result <- CytoConverter(sample_data)
#' graph_data <- cyto_graph(result$Result)
#' 
#' # Use with different reference genome
#' graph_data <- cyto_graph(result$Result, ref_list="hg19")
#' 
...
#' 1. Loads or validates reference cytoband data
#' 2. Identifies overlapping loss regions for "double loss" detection
#' 3. Transforms genomic coordinates to plotting coordinates
#' 4. Calculates sample positioning for vertical layout
#' 5. Prepares rectangle coordinates for gain/loss visualization
#' 
#' Color coding in output:
#' - Gains: Represented for red plotting
#' - Losses: Represented for blue plotting  
#' - Double losses: Represented for orange plotting
#'
#' @export
#' 
#' @seealso 
#' \code{\link{plot_cyto_graph}} for plotting the processed data
