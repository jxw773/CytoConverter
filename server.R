  ...
  function(input, output) {
    ...
    fusionDatasetInput <- reactive({
      switch(input$dataset,"Both"=list(rbind(CytotableString()[[3]],CytotableFile()[[3]]),rbind(CytotableString()[[2]],CytotableFile()[[2]])),
             "Text"= list(CytotableString()[[3]], CytotableString()[[2]]),
             "File"= list(CytotableFile()[[3]], CytotableFile()[[2]]))
    })
    ...
    ##download data   table
      ...
      output$downloadFusionData <- downloadHandler(
        ...
        content = function(file) {
          write.table(fusionDatasetInput()[[1]], file,quote=F,sep='\t',row.names = F)
        }
      ...
      make_plot_string<-reactive({ 
        cyto_graph(CytotableString()[[1]],input$radio,input$radio5,CytotableString()[[4]])
        
      })
      
      ##function to plot stuff-table
      make_plot_table<-reactive({ 
        cyto_graph(CytotableFile()[[1]],input$radio,input$radio5,CytotableFile()[[4]])
      ...
      output$fusiontable<-renderTable({CytotableString()[[3]]})
      output$fusionfiletable<-renderTable({CytotableFile()[[3]]}) 
      ##change this to recive stuff from make plot string and make plot table
      output$singleplot<-renderPlot({
          
      
        
        if((!is.null(CytotableString()[[1]]) && (nrow(CytotableString()[[1]]) >= 1 | !is.null(CytotableString()[[4]]))))
        {
          
          list_from_cyto<-cyto_graph(cyto_list=CytotableString()[[1]],input$radio,input$radio5,CytotableString()[[4]])
      ...
      output$tableplot<-renderPlot({
        ...
        if(!is.null(CytotableFile()[[3]]) && (nrow(CytotableFile()[[1]]) >= 1 | !is.null(CytotableFile()[[4]])))
        {
           
              list_from_cyto<-cyto_graph(cyto_list=CytotableFile()[[1]],input$radio,input$radio5,CytotableFile()[[4]])
      ...
      output$singleplot_fus<-renderPlot({
      ...
      output$tableplot_fus<-renderPlot({
